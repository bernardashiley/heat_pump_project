...
4. All essential MCS fields are present, non-null, and physically valid:
   - `floor_area` > 10 m²
   - `heat_loss` is in the range 0.5 kW to 50 kW (500 W to 50,000 W). The upper bound was added after the first full evaluation surfaced three cases with heat_loss values of 5,188-7,000 kW, indicating unit errors in the source MCS data.
   - `-15 °C < design_temp < 5 °C`
   - `flow_temp` > 20 °C
   - `hp_output` > 1 kW
   - `latitude` and `longitude` are present and non-null

   Amendment 1 (10 June 2026, pre-evaluation): this criterion was tightened from "present and non-null" after a pre-evaluation data audit found 47 cases with non-positive `heat_loss`, 18 with `design_temp >= 0`, and 6 with `floor_area <= 0`. These cases reflect data-entry errors in the MCS reports and would crash Pydantic validation in the model. The amendment was applied before any evaluation metrics were computed.

   Amendment 2 (10 June 2026, post first full evaluation): the upper bound on `heat_loss` was added after the first full evaluation surfaced three cases with `heat_loss` values of 5,188-7,000 kW. These are source-data unit errors, not plausible domestic or small commercial heat losses. The amendment removes invalid source data from the evaluation set; it does not change the model.
...
### 6.8 Known model issue surfaced by this evaluation

The first full evaluation (10 June 2026) revealed that the v1 model systematically underpredicts annual electricity by approximately 30% on the measured-input subset, with 72.8% of cases falling above the model's 90th percentile. Diagnostic audits identified the model's October-March winter window for climate data as a strong candidate cause: real UK heat pumps consume electricity during shoulder seasons (September, April, May) that the model does not currently incorporate.

This is a finding of the evaluation, not a pre-registered hypothesis. It is documented here for transparency. Any model change made in response to this finding constitutes a v2 model and requires a separate pre-registered evaluation. The numbers reported below were generated by the v1 model as committed at the time of evaluation, without post-hoc adjustment.
